    ...
    for result in rdr.deserialize() {
        ...
        if !name.is_empty() {
            pubs.push(ImportPub {
                ...
                untappd_verified: record
                    .get("untappd_verified")
                    .map(|s| s == "true")
                    .unwrap_or(false),
                years,
            });
...
pub fn parse_parquet(path: &str) -> Result<Vec<ImportPub>> {
    ...
    for batch_res in reader {
        ...
        let name_col = batch
            ...
            .as_any()
            .downcast_ref::<StringArray>()
            .context("Missing name col")?;
        let addr_col = batch
            .column(2)
            .as_any()
            .downcast_ref::<StringArray>()
            .context("Missing address col")?;
        let town_col = batch
            .column(3)
            .as_any()
            .downcast_ref::<StringArray>()
            .context("Missing town col")?;
        let region_col = batch
            .column(4)
            .as_any()
            .downcast_ref::<StringArray>()
            .context("Missing region (formerly county) col")?;
        let postcode_col = batch
            .column(5)
            .as_any()
            .downcast_ref::<StringArray>()
            .context("Missing postcode col")?;
        let closed_col = batch
            .column(6)
            .as_any()
            .downcast_ref::<BooleanArray>()
            .context("Missing closed col")?;
        let lat_col = batch
            .column(7)
            .as_any()
            .downcast_ref::<Float64Array>()
            .context("Missing lat col")?;
        let lon_col = batch
            .column(8)
            .as_any()
            .downcast_ref::<Float64Array>()
            .context("Missing lon col")?;
        ...
        for i in 0..batch.num_rows() {
            ...
            if !name.is_empty() {
                pubs.push(ImportPub {
                    ...
                    lat: if lat_col.is_null(i) {
                        None
                    } else {
                        Some(lat_col.value(i))
                    },
                    lon: if lon_col.is_null(i) {
                        None
                    } else {
                        Some(lon_col.value(i))
                    },
...
mod tests {
    ...
    fn test_parse_csv_basic() -> Result<()> {
        ...
        writeln!(
            file,
            "name,address,town,region,country_code,postcode,closed,years"
        )?;
        writeln!(file, "\" The Bell \",\" 2 Main St \",\" Town \",\" Region \",\"W\",\" PC2 \",false,\"2021;2022\"")?;
        writeln!(
            file,
            "\"\",\"Hidden\",\"Town\",\"Region\",\"\",\"PC\",true,\"\""
        )?;
