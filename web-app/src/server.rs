...
pub async fn get_pubs(query: String) -> Result<Vec<PubSummary>, ServerFnError> {
    ...
    let pubs = sqlx::query_as!(
        PubSummary,
        r#"SELECT id, name, town, county, postcode, closed 
           FROM pubs 
...
pub async fn get_pub_detail(id: Uuid) -> Result<PubDetail, ServerFnError> {
    ...
    let pub_info = sqlx::query!(
        r#"SELECT id, name, address, town, county, postcode, closed, 
    ...
    Ok(PubDetail {
        id: pub_info.id,
        name: pub_info.name,
        address: pub_info.address.unwrap_or_default(),
        town: pub_info.town.unwrap_or_default(),
        county: pub_info.county.unwrap_or_default(),
        postcode: pub_info.postcode.unwrap_or_default(),
        closed: pub_info.closed.unwrap_or_default(),
