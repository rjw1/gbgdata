...
#[component]
pub fn MapView(#[prop(into)] pubs: Signal<Vec<PubSummary>>) -> impl IntoView {
        ...
        Effect::new(move |_| {
            ...
            if let Some(container) = map_container.get() {
                ...
                if !has_map {
                    ...
                    let map = js_sys::Reflect::get(&l, &JsValue::from_str("map"))
                        .unwrap()
                        .dyn_into::<js_sys::Function>()
                        .unwrap()
                        .call1(&JsValue::NULL, &container.clone().into())
                        .unwrap();
                    ...
                    let set_view = js_sys::Reflect::get(&map, &JsValue::from_str("setView"))
                        .unwrap()
                        .dyn_into::<js_sys::Function>()
                        .unwrap();
                    ...
                    set_view
                        .call2(&map, &coords, &JsValue::from_f64(zoom as f64))
                        .unwrap();

                    // Add tile layer
                    let tile_layer = js_sys::Reflect::get(&l, &JsValue::from_str("tileLayer"))
                        .unwrap()
                        .dyn_into::<js_sys::Function>()
                        .unwrap()
                        .call1(
                    ...
                    let marker_fn = js_sys::Reflect::get(&l, &JsValue::from_str("marker"))
                        .unwrap()
                        .dyn_into::<js_sys::Function>()
                        .unwrap();
                    let _bind_popup =
                    ...
                    for p in list.iter().filter(|p| p.lat.is_some() && p.lon.is_some()) {
                        ...
                        let set_popup =
                            js_sys::Reflect::get(&marker, &JsValue::from_str("bindPopup"))
                                ...
                                .unwrap();
                        set_popup
                            .call1(&marker, &JsValue::from_str(&popup_content))
                            .unwrap();
